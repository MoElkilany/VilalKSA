...
struct CancellationTermsPage: View {
    @EnvironmentObject var pilot: UIPilot<MyAccountDestination>
    @State var state: AppState = .success
    
    var body: some View {

        VilalKSAContainer(state: $state,titlePage: R.string.localizable.cancellation_Terms.localized, tryAgainAction: nil,backAction:{
            pilot.pop()
        },content: {
            VStack {
