...
struct FavoritesPage: View {
    ...
    var body: some View {
        ...
        StateOnContentView(state: $viewModel.state, tryAgainAction: {
            viewModel.getFav()
        },stateError:"",content: {
            VStack{
                HStack{
                ...
                ScrollView(showsIndicators:false){
                    VStack{
                        ...
                        ForEach(self.viewModel.favoriteList ?? [], id: \.self ) { item in
                            ...
                            .onTapGesture {
                                pilot.push(.adsDetailsPage(id: String(item.id ?? 0 ), type: .favorite))
                            }
                        }
                    }
                }
            }
        }, titlePage:R.string.localizable.favorites.localized)
